...
This workspace contains the Angular frontend for myTBI. If you are new to frontend development, treat this app as the browser-facing layer of the system: it renders UI, responds to user interaction, and calls the API.
...
|  |- theme.scss               Triangle Baha'i Institute Material theme (light + dark)
...
|     |- tools/                Tools feature routes (list, detail, etc.)
|     |- jwt/                  JWT callback route feature
|     |- api/generated/        Auto-generated HTTP client and models
|     `- api/models.ts         Domain-friendly type aliases (Tool, User, etc.)
...
The frontend uses Angular Material for UI components and Tailwind CSS  for layout utilities. Theming follows the Triangle Baha'i Institute brand palette.

### App Shell

The `Layout` component in `src/app/layout/` provides a responsive shell:

- **Desktop**: A full-width toolbar on top with page title, theme toggle, and auth controls. 
- **Mobile**: 
...
2. Navigate the user to the next useful destination (e.g. Event dashboard after checking in into an event).
...
- The API produces an OpenAPI 3.1 spec with clean operation IDs (e.g. `create_event`, not `create_event_api_courses_post`).
...
- **Domain types:** Import domain types from `api/models` (e.g. `Event`, `User`, `Participants`), not from `api/generated/models/`. The barrel file `src/app/api/models.ts` maps generated names (`EventResponse`, `UserProfile`) to clean domain names.
...
2. Open the route's component folder (e.g. `src/app/courses/event-list/`).
3. Check any supporting services (`event.service.ts`, `page-title.service.ts`).
